...
description: 'Tap a bird card to the NFC reader to hear one of its calls, attempt to identify the bird from its vocalizations alone!'
created: 2026-07-25
published: Sat, 25 Jul 2026 10:50:00 EST
header: '<style>
.blog {
  img {
    max-width: 90%;
...
Each bird card has an NFC tag associating it with three to ten audio recordings of that bird's call. When you tap a card on the Birduino's card reader, one of those recordings will play.[^shuffle] 

To test yourself on recall, keep the front of the cards hidden and try to guess the bird from the call alone.

[^shuffle]: The recordings are shuffled randomly each time the card triggers playback, so you can't [:just-flag:](../just-flag/) get good at recognizing the ambient sound of a bird's recording instead of the bird's call itself.
...
Construction of the Birduino involved setting up a [TonUINO](https://www.voss.earth/tonuino/), designing and assembling a clear acrylic housing for it, and mapping a card deck of birds to their respective recordings as loaded onto the TonUINO.
...
The TonUINO community's [comprehensive FAQ](https://discourse.voss.earth/t/haeufige-fragen-faq/13282) came in handy for several aspects of this set up, as did Google Translate (!).

Our compact and portable soldering station made soldering a delight! (Thanks, AF!)

![Photo of in-progress soldering](./assets/solder1.png)

![Another photo of in-progress soldering](./assets/solder3.png)

Here's my resultant schematic; I colored each wire as I selected it.[^color]
...
> 
> Similarly, my choice of wires was not ideal. I chose the rainbow wires, as they had connection jacks that slotted nicely into the headers that were pre-soldered onto kit various components. This would have been great had I been working with a breadboard and simply inserting all connector pins into slots, but instead I needed to solder the other end of those wires to the proto board---which required cutting the pins off and soldering them directly. Something about these wires did *not* solder well, which gave me a bunch of messy connections, and more inadvertent bridging that I had to debug and fix.</br></br>
> 
> Learn from my mistakes, and get yourself a proto board with suitable properties, and do not use these wires if you need to do any soldering!

</details>

## Software: Configuring and loading the TonUINO application

The TonUINO application is written in C++; I cloned [the repository](https://github.com/tonuino/TonUINO-TNG) and uncommented various sections to match my hardware configuration. I then used the Arduino IDE to load it onto the microcontroller.[^specifics]

[^specifics]: Configuration specifics, in case you're making a Birduino too:
...
For audio, I purchased [_The Cornell Guide to Bird Sounds: United States and Canada_](https://www.macaulaylibrary.org/product/the-cornell-guide-to-bird-sounds-us-and-canada/):
...
Programming the NFC tags on the bird cards ended up being straightforward, thanks to Max Mehl's [TonUINO Cards Manager](https://github.com/mxmehl/tonuino-cards-manager) project, which maps a manifest of audio files to their corresponding TonUINO-card-ready byte strings, which is then written to NFC tags to create the set of TonUINO cards. 

To make the manifest for the Birduino, I listed out the birds (in the order of my card deck) and then iterated through them, matching any audio file names from the Cornell Guide.
...
My full script and manifest are [here](https://github.com/hannahilea/notebook-playground/tree/main/tonuino-birds), should you want to make your own.
...
[^fast]: "This process" is the operative phrase here; before I had AF dig her old Android phone out of storage to facilitate this helper application, I first tried using a bunch of unhelpful RFID-programming apps from the Apple app store. The programs exist, but are designed to be "user friendly" and program URLs or text blobs or files.  Too user friendly. I couldn't find one that would support a basic binary string as required by the TonUINO.
...
Using parametric CAD program [Cuttle.xyz](../cuttle-obsession/), I modified a basic press-fit box to give it cutouts for the knob and speaker, and overall dimensions such that the prototyping board could act as the box's top. 

![Vectorized layout of box to cut.](./assets/cad-box.png)
The Cuttle design is [here](https://cuttle.xyz/@hannahilea/Birduino-enclosure-xwsytzmwwLnc).

I designed a second box to hold the cards, and a tray with inserts for both boxes to fit into so that the audio player could be presented along with its cards.

![Vectorized layout of tray + rest to cut.](./assets/cad-tray.png)
The Cuttle design is [here](https://cuttle.xyz/@hannahilea/Birduino-Tray-MhnQCGlMyedq)
...
Even though the enclosure is technically a press-fit box, I wanted to glue it for longevity.[^longevity] Choosing an adhesive was non-trivial, as I needed to find a glue that didn't make the acrylic cloudy. After testing several different options, we[^af] settled on _Clear Gorilla Glue Gel_.[^gorilla] 

[^longevity]: Also because the aforementioned tangle of wires crammed were pressing outward in an attempt to break free, acting to press *un*fit!

[^af]: "We" because I did not do this step alone: AF was the MVP of the glue process: when she saw I was losing my mind over it, she took over glue research and procurement.
...
[^gorilla]: No thanks to the Gorilla Glue's branding, which includes many different form factors and formulations of glue, all similarly named despite different chemical differences. 
    ...
    One resource that did prove useful was Danielle Feliciano Wethington's [_The Acrylic Gluing Guide_](https://daniellefeliciano.com/gluingacrylic/).
...
So much for a light and breezy project! It was satisfying to complete, though, and is fun to play with.
