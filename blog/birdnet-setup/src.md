...
- Access to **an SD card writer**, in dongle form [ [MicroCenter, $18](https://www.microcenter.com/product/467960/iogear-superspeed-usb-30-sd-microsd-card-reader-writer) ] if there isn't one already built into your computer.
...
1. Plug the SD card writer into your computer, and put the microSD card into it. (The microSD card almost certainly came with an adapter that allows it to fit into the SD card writer slot.)

    ![Photo of SD card in writer](./assets/photo-sdcard.png)
...
4. You'll be prompted to optionally "Apply OS customization settings?", for which you should select **`EDIT SETTINGS`**. On this page, set:
...
5. You'll be reprompted to "Apply OS customization settings?"; now select `YES`. You might get a scary looking message asking you to confirm that any existing data on the card will be overwritten; select `YES` to continue. If prompted for a password, use the password for the computer you're running the configuration from---not any of the passwords we've set for the Pi. 

    ![Screenshot of additional selection dialog, with choices matching those described in the above text.](./assets/pi-image-4.png)

    The Raspberry Pi Imager will now proceed to write the operating system and configuration settings to your microSD card! It may take a few minutes, but the progress bar will tell you when its done, and the microSD card will be automatically disconnected from your computer ("ejected") once the writing is complete. At this point it is safe to remove the card from the reader!

6. Remove the microSD card from the writer and put it into the microSD card slot in the Raspberry Pi 2 Zero W: 

    ![Photo of MicroSD card in Pi 2 Zero W](./assets/sd.png)

8. Connect the power cable to one of the Pi's micro USB ports, and plug its other end into the wall. Once powered, the Pi will turn on automatically; you'll be able to tell, because a green light will start flashing on the Pi. This may take a few moments!

The Pi will use the Wi-Fi credentials you set earlier to automatically connect to your wireless network. We can (and will!) do the rest of the setup wirelessly!
...
1. On your personal computer, open up your terminal application---if you're on a Mac, this is literally called "Terminal"; on Windows it's called "Windows Terminal"---and type
    ...
    For example, to SSH onto the device in my screenshots above, I type `ssh birbwatcher@birdnet-hannex.local`.

    ![Screenshot of ssh connection window.](./assets/ssh.png)

3. If prompted to continue connecting, type `yes` and hit `enter`. (This will only happen the first time your computer connects to the new Pi.)

2. When prompted for a password, type the `<pi_password>` from above (without the angle brackets!!) and hit `enter`.

That's it, you're on! Any commands you type into your terminal window will now be run *on your Raspberry Pi*. 😮

Oh, and if you want to close the SSH connection? Type `exit` and then hit `enter`, or just close the terminal window.
...
Look at the start of each line: when you're on the Pi, the "prompt"---the default text at the start of the line in your terminal---will be `<pi_username>@birdnet-<pi_identifier>`. When you're on your personal machine, it will be something else, probably the name of your computer unless you've customized it to look different. 

![Screenshot demonstrating opening and closing an SSH connection.](./assets/ssh2.png)

In the screenshot, my local prompt is the path to my working directory "~" in a blue arrow, and the prompt on the Pi is the white `birdnet-hannex`. (The text styling may look different for yours, but the concept is the same.)
...
In this step, we'll download and run the installer for the BirdNET-Pi application, and then change some of the Pi's configuration for storage limits, restart behavior, and logging. Some of the following is going to seem a little silly, but it works. ¯\\\_(ツ)\_/¯ Bear with me! 

If you're prompted for a password at any point, use the `<pi_password>` you set previously. When a step says "run", that means to type or copy the given text into the terminal and then hit `enter`.

1. In your remote terminal---that is, the one where you've SSH'd onto your Pi---run
    ```
    sudo nano /etc/rc.local
    ```

    This will open up a new file (which will be saved to `/etc/rc.local`) in a program called nano, which is a text editor that runs in the terminal. Copy and paste the following into it:
    ```
    #!/bin/sh -e
    iw wlan0 set power_save off
    exit 0
    ```
    Select `ctrl` and `x` at the same time to close the file, then `Y` followed by `enter` to save this content to the new file. After the file saves you'll be returned back to your command prompt.

    Want to confirm that you write and saved the correct content to this file? Run
    ```
    cat /etc/rc.local
    ```
    to print the contents of that `/etc/rc.local` file in the terminal. If the content matches the above text, you're good to go---if it doesn't, simply try again: do `sudo nano /etc/rc.local` to reopen the file in your editor, and edit it so that it matches!

2. Run `sudo chmod u+x /etc/rc.local` in the terminal. This makes the new file you just created executable, i.e., allows the computer to run it.

3. Update the swap space limits! Run 
    ```
    sudo nano /etc/dphys-swapfile
    ```
    to open the file `/etc/dphys-swapfile` in nano. Then press the down/up arrow keys to find and update the lines that contain `CONF_SWAPSIZE` and `CONF_MAXSWAP`. For each line, edit it to:
    
    1. Delete the `#` from the start of the line, if present, and
    2. Update their values to be `CONF_SWAPSIZE=2048` and `CONF_MAXSWAP=4096`, respectively.

    This is what part of that file should look like, after you've edited it: 

    ![Screenshot of text file with the above settings set.](./assets/screenshot-swapfile.png)

    *A `#` at the start of the line means that the line is commented out---which means that the changes on that line will be ignored. We do not want them to be ignored, so we delete that `#`!*

    Once more, hit `ctrl` and `x` together, followed by `Y`, followed by `enter`, to save these changes; once more, you'll be returned to the terminal.
...
5. Wait for a moment or two for the Pi to reboot, then SSH back onto it:
    ...
    (But you knew that already by now, right? :) )

6. Okay, this is the bit that is going to feel silly. Just go with it. In your remote terminal, run
...
7. After you've gotten the error, run the following at the prompt:
    ...
    ...when it eventually succeeds (eventually! This step is slow, lots of things happen during installation) the Pi will reboot.

8. ...SSH back onto it again! I bet you remember the command this time. 

    *You can also probably use the up-arrows in your terminal to page back through the previous command history in your terminal, which at this point definitely contains your SSH command! You can hit `enter` on any item in the command history to rerun it.*
...
9. Run
    ...
    - Replace `#SystemMaxUse=` with `SystemMaxUse=200M` and then

    - Save the file ( `ctrl+x` , `Y`, `enter`!).

10. Run `systemctl restart systemd-journald`. This will prompt you to enter your password `<pi_password>` when prompted.

At this point, you've installed the BirdNET-Pi application! Congrats!

### Task 4: Configure the BirdNET-Pi application

The BirdNET-Pi application consists of two components: a classification pipeline that constantly analyzes recorded audio for bird sounds, and a small web server that hosts a website to visualize those detections. The website also lets users configure the application's various settings. That's what we'll be doing in this task!
...
6. If this is your first time using Tailscale, also install it on whatever device you want to use to access the BirdNET-Pi---e.g., your personal computer, or your phone. Steps for these different devices are available [here](https://tailscale.com/kb/1347/installation).
